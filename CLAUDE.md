...
| Web UI | **Minimum viable shipped.** Vanilla-JS SPA embedded in the binary at compile time (assets/web/). Inbox + Issues list + Issue detail + Schedule + Daemon screens. Real-time updates via SSE. Deferred to subsequent slices: full mutation UI from detail screen (transition picker / blocker / priority dropdowns / edit / dep add-remove), Dependencies graph (R7), Config screen (R10), URL-state filter persistence (R5.4). |
